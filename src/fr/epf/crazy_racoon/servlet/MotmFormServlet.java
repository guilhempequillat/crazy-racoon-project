...
public class MotmFormServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("grade") != null) {
			...
			loadTemplate(request);
			request.getSession().setAttribute("gradeExist", true);
		} else {
			request.getSession().setAttribute("gradeExist", false);
		}
	...
	public void checkMotmDate(Long idUser) {
		...
		for (int i = 0; i < listMotm.size(); i++) {
			...
			if (calendarMotm.get(Calendar.MONTH) == calendar.get(Calendar.MONTH)
					&& calendarMotm.get(Calendar.YEAR) == calendar.get(Calendar.YEAR)) {
	...
	public void saveMotmInSession(HttpServletRequest request, Motm motm) {
		...
		request.getSession().setAttribute("year",
				(int) (calendar.get(Calendar.YEAR) - calendarMotm.get(Calendar.YEAR)));
		request.getSession().setAttribute("month",
				(int) (calendar.get(Calendar.MONTH) - calendarMotm.get(Calendar.MONTH)));
		request.getSession().setAttribute("day",
				(int) (calendar.get(Calendar.DAY_OF_MONTH) - calendarMotm.get(Calendar.DAY_OF_MONTH)));
	}
